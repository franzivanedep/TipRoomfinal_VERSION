...
export function Acourseassign() {
 ...
 useEffect(() => {
  fetch('http://localhost:6969/courses')
  ...
  const assignInstructor = () => {
    if (selectedStudentIds.length >  0 && selectedProfessorId !== null) {
      ...
      const assignmentPayload = {
        p_ids: selectedStudentIds,
        c_id: selectedProfessorId
      };
      ...
      fetch('http://localhost:6969/coursesrelation', requestOptions)
        .then(response => response.json())
 ...
 <AvatarFallback>{professors.first_name ? professors.first_name[0] : ''}{professors.last_name ? professors.last_name[0] : ''}</AvatarFallback>
  ...
  type="checkbox"
  className="ml-auto"
  id={`professors-${professors.p_id}`}
  checked={selectedStudentIds.includes(professors.p_id)}
  
  onChange={() => handleStudentSelection(professors.p_id)}
/>
                </div>
              ))}
            </div>
